...
@click.argument('geofile_column_name', type=str)
@click.argument('new_column_name', type=str)
@click.argument('output_filepath', type=click.Path())
def main(input_filepath, enrichment_filepath, geofile_column_name, new_column_name, output_filepath):
    ...
    geo_clusters = gpd.read_file(enrichment_filepath)
    ...
    def assign_geo_designation_tract(row):
        ...
        bools = [geom.contains(row['geometry']) for geom in geo_clusters['geometry']]
        if True in bools:
            return geo_clusters.iloc[bools.index(True)][geofile_column_name]
        else:
            return np.nan

    logger.info('Adding Geo-labeling')
    dc_df[new_column_name] = dc_df.apply(assign_geo_designation_tract, axis='columns')
